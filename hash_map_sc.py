...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        # Calculate the index using the hash function modulo table capacity, then access the LinkedList at that index
    ...
    def _calculate_index(self, key: str) -> int:
        """
        Returns the index of the associated key. Calculated by taking the hash functions result modulo HashMap capacity.

        :param key:     Key (string) that we are calculating an index (bucket) for.

        :return:        Integer representing the index (bucket) associated with a given key.
        """
        # Calculate and return index (hash function output for key modulo HashMap capacity)
        return self._hash_function(key) % self._capacity

    def _get_bucket(self, key: str) -> LinkedList:
        """
        !! Get further and then consider if this is worth it. Cleaner, but no real difference.
        """
